    ...
    public partial class ItemsControl : UserControl
    {
            ...
            if (Clipboard.ContainsImage())
            {
                ...
                using (Bitmap bmp = new Bitmap(Clipboard.GetImage()))
                {
                    ...
                    if (index >= 0 && index < Art.GetMaxItemId())
                    {
                        ...
                        // Iteriere durch jedes Pixel des Bildes
                        for (int x = 0; x < bmp.Width; x++)
                        {
                            for (int y = 0; y < bmp.Height; y++)
                            {
                                ...
                                // Überprüfe, ob die Farbe des aktuellen Pixels eine der Farben ist, die umgewandelt werden sollen
                                if (colorsToConvert.Contains(pixelColor))
                                {   // Setze die Farbe des aktuellen Pixels auf Schwarz (#000000)
                ...
                using (Bitmap bmp = new Bitmap(Clipboard.GetImage()))
                {
                    ...
                    Color.FromArgb(255, 255, 255), // #FFFFFF => #000000
                     Color.FromArgb(254, 254, 254)  // #FEFEFE => #000000
                        ...
                        // Save the final bitmap to a file in the same directory as the main program
                        string filePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Testbild.bmp");
        ...
        /*private void importToolStripclipboardMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Clipboard.ContainsImage())
            {
                ...
                {
                    { Color.FromArgb(211, 211, 211), Color.FromArgb(255, 255, 255) }, // #D3D3D3 -> #FFFFFF
                    { Color.FromArgb(0, 0, 0), Color.FromArgb(255, 255, 255) }  // #000000 -> #FFFFFF
                };
                        ...
                        for (int x = 0; x < bmp.Width; x++)
                        {
                            for (int y = 0; y < bmp.Height; y++)
                            {
                                ...
                                if (colorsToReplace.ContainsKey(pixelColor))
                                {
                                    ...
                                    newBmp.SetPixel(x, y, colorsToReplace[pixelColor]);
                                }
                                else
                                {
                                    // Set the color of the current pixel to the color of the original image
                        ...
                        // Convert the image to a 16-bit color depth
                        Bitmap bmp16bit = new Bitmap(newBmp.Width, newBmp.Height, System.Drawing.Imaging.PixelFormat.Format16bppRgb555);
                        using (Graphics g = Graphics.FromImage(bmp16bit))
                        {
                            g.DrawImage(newBmp, new Rectangle(0, 0, bmp16bit.Width, bmp16bit.Height));
                        }

                        // Replace the image at the specified index
                        Art.ReplaceStatic(index, bmp16bit);
                        ControlEvents.FireItemChangeEvent(this, index);
